...
check "POST /api/todos"             200 "$(code -X POST -H 'Content-Type: application/json' -d '{"title":"t1","description":"d","status":"doing"}' $BASE/api/todos)"
check "POST title のみ"              200 "$(code -X POST -H 'Content-Type: application/json' -d '{"title":"t2"}' $BASE/api/todos)"
check "title のみは status=todo"     "yes" "$(curl -s "$BASE/api/todos?status=todo" | grep -q '"title":"t2"' && echo yes || echo no)"
check "POST 不正ボディ -> 422"       422 "$(code -X POST -H 'Content-Type: application/json' -d '{"nope":1}' $BASE/api/todos)"
check "POST 不正 status -> 422"      422 "$(code -X POST -H 'Content-Type: application/json' -d '{"title":"x","status":"nope"}' $BASE/api/todos)"
check "旧 completed:true は done"    "yes" "$(curl -s -X POST -H 'Content-Type: application/json' -d '{"title":"legacy","completed":true}' $BASE/api/todos | grep -q '"status":"done"' && echo yes || echo no)"
check "status=doing フィルタに出る"   "yes" "$(curl -s "$BASE/api/todos?status=doing" | grep -q '"title":"t1"' && echo yes || echo no)"
check "フィルタ外は出ない"            "no" "$(curl -s "$BASE/api/todos?status=doing" | grep -q '"title":"t2"' && echo yes || echo no)"
check "不正 status フィルタ -> 422"   422 "$(code "$BASE/api/todos?status=nope")"
